-- Corpo das funções do Overview, reescritas em plpgsql com SQL dinâmico.
-- Separado de 20260801180000 (que traz o índice, as auxiliares e o porquê)
-- só para cada arquivo ficar legível. Já aplicadas no projeto padrão.
--
-- A montagem do texto da consulta em runtime é o ponto central: cada filtro só
-- entra na string quando está preenchido. Isso não é micro-otimização — é a
-- única forma de manter as colunas caras fora do PLANO, já que o Postgres
-- decide o que ler antes de executar e nenhum curto-circuito evita a leitura.
--
-- Continuam SECURITY INVOKER: respeitam a RLS de quem chama, e `anon` recebe
-- 42501 ao chamá-las — que é o erro que faz a tela de login aparecer.
